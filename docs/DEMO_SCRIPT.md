...
One continuous story, not a feature tour: **the standard KleidiAI benchmark is measured against
the wrong baseline, and here is a harness that catches it — including catching itself.**
...
**[1:20–1:50] Proof the kernels actually ran.**
...
**[1:50–2:20] The harness catching a lie. ← the moment**
...
> multiplier it prints **within noise — not claimed**. A benchmark that can't say *no* isn't
> measuring anything."
...
`ttft`, `throughput`, `profile`, `autotune`, `perplexity`, the quant/KV/micro-batch sweeps and
the `versions/` ladder are all real and all documented — and none of them belong here. They make
the video a feature tour, which is the standard way an infrastructure project loses. One claim,
proved, is the whole job.
