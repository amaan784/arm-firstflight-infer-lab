...
This document states how every number in the report is produced, so anyone can reproduce
or challenge it.

## What we measure (and why)

The headline metric is **prefill latency / time-to-first-token (TTFT)** as a function of
context length. Agentic and RAG workloads put long contexts in the prompt, so the latency
before any token appears is dominated by **prompt processing (prefill)**, not generation.
Long-context prefill scaling is therefore a first-class axis.
...
processing) and `prompt_n` (tokens prefilled). This is the server's own measurement and
...
full prefix; the warm turn processes only the new question, so `prompt_n` drops from
thousands of tokens to dozens, and `prompt_ms` with it. Both turns are reported side by side
(`bench/results/ttft_*.json`), using the server's measurements.
...
- **Repeats + variance.** `repeats` timed runs per config (default 5). We report llama-bench's
  mean (`avg_ts`) ± stddev across repetitions, not a single number.
- **A measured noise floor, and a gate that uses it.** The `noise-floor` experiment runs the
  same build with the same config under two labels. Any apparent speedup between them is
  runner noise, not the treatment, so that spread is the bar a real delta must clear. The
  report computes it and refuses to headline a speedup that sits inside it, printing
  "within noise / not claimed" instead of a multiplier.
...
- **Pinned environment.** llama.cpp pinned to a specific `b####` release tag; deps pinned in
  ...
  and `-DGGML_CPU_KLEIDIAI=ON` is Arm's documented enable flag.
...
> active by checking the load log for:
...
- **Built-in probe (default):** a small fixed exact-match Q&A set run through `llama-completion`:
  ...
  is flat there by construction. Perplexity covers those cases:
- **Perplexity (`firstflight perplexity` / automatic in experiments):** `llama-perplexity`
  over a fixed corpus (default: this repo's own docs; the text is identical across the configs
...
Opt-in, so the core ships without it.

## Profiling (Arm Performix)

`firstflight profile` runs Arm Performix (`apx`) against a representative prefill command and
surfaces the top self-time hotspot functions into the report. The `apx` flow (sourced
...
with a message and the report omits the section.
...
`kleidiai-null-control` runs the KleidiAI build against a Q4_K_M model, which
...
kernels room to show a delta of their own. Reporting one quant alone would misrepresent the
mechanism.
