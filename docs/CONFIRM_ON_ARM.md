# Confirm-on-Arm checklist (verify, don't invent)
...
Checked against authoritative sources (dates noted) and baked in — still re-confirm if you pin
...
6. **Pinning CPU mask** — `configs/experiments.yaml` `pinning` uses `cpu_mask: "0x0f"`
   (cores 0–3); set it for the core count of the box you actually pin on.
